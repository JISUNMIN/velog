...
<p>최신 브라우저(Chrome·Edge·Safari 등)는 최신 JS 문법을 대부분 네이티브로 지원하기 때문에
개발 환경(dev)에서는 다운레벨링이 필요하지 않습니다.</p>
<p>하지만 <strong>옛날 브라우저(IE, 오래된 Safari 등)</strong>는 최신 문법을 이해하지 못하기 때문에
배포용(build)에서는 호환성을 위해 다운레벨링을 수행합니다.</p>
<p>즉, <strong>최신 브라우저는 최신 JS를 그대로 읽을 수 있지만,
옛날 브라우저는 이해하지 못하므로 모든 브라우저에서 동작하도록 변환이 필요한 것입니다.</strong></p>
<hr />
