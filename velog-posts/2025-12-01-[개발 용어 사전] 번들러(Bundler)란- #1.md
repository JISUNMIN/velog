<p>프론트엔드 개발을 하다 보면<br />번들러, 트랜스파일러, 빌드 도구처럼<br />비슷한 용어들이 한꺼번에 등장해 헷갈릴 때가 많습니다.<br />그래서 이번 기회에 이러한 개념들을 하나씩 정리해보려고 하며,<br />이번 글은 그중에서도 가장 기본이 되는 <strong>번들러(Bundler)</strong>부터 살펴보겠습니다.</p>
<hr />
<p><strong>번들러(Bundler)</strong>는  
여러 파일을 하나로 묶어 브라우저가 더 빠르고 효율적으로 실행할 수 있게 해주는 도구입니다.</p>
...
<p>라고 이해할 수 있습니다.</p>
<p>참고로, <strong>번들러와 빌드 도구는 실제 개발 환경에서 함께 사용되기 때문에
둘의 역할이 모호하게 느껴질 때가 많습니다.</strong>
특히 CSS·이미지·폰트 같은 리소스 처리 과정은 번들러가 직접 수행하는 기능이 아니라,
<strong>빌드 도구가 이러한 리소스를 JS 모듈 형태로 변환해 주어야만 번들러가 최종 번들링 과정에 포함시킬 수 있습니다.</strong></p>
<p>이 글에서는 개념을 명확히 하기 위해
<strong>번들러가 실제로 맡는 기능(JS 모듈 번들링)</strong>에만 초점을 맞추고 정리했습니다.</p>
<p>또한 최근에는 번들링과 빌드를 동시에 수행하는 <strong>통합형 도구도 많이 등장</strong>하면서, 각각의 도구 역할이 더욱 혼동되기 쉽습니다.
이 때문에 앞으로 정리에서 다루게 될 도구 중 일부는 번들러에도 속하고, 빌드 도구이기도 하며, 때로는 트랜스파일러 역할까지 
수행하는 등 여러 범주에 동시에 등장할 수 있다는 점을 함께 참고해 주시면 좋겠습니다.</p>
<hr />
...
<p>➡ 그래서 필요한 것이 <strong>번들러(Bundler)</strong> 입니다.</p>
...
<p>프론트엔드에서 <strong>빌드(Build)</strong> 는 프로젝트를 실제 배포 가능한 형태로 만드는 전체 과정이고,<br /><strong>번들링(Bundling)</strong> 은 그 안에서 여러 파일(JS/CSS/이미지 등)을 묶고 최적화하는 단계입니다.</p>
<pre><code class="language-text">빌드(build)
├─ 번들링(bundling)         → 여러 JS 모듈을 하나/소수의 번들로 묶기
...
<pre><code>최종 빌드(프로덕션)를 담당하는 번들러
Vite의 빌드 단계에서도 Rollup이 사용됨</code></pre><h4 id="장점-1">장점</h4>
<ul>
...
<pre><code>esbuild = 초고속 번들러 + 트랜스파일러
Vite의 개발(dev) 모드에서 사용됨</code></pre><h4 id="장점-2">장점</h4>
<ul>
...
<pre><code>Parcel = 번들러 + 빌드 시스템(트랜스파일링 자동 포함)</code></pre><p>설정 없이도 거의 모든 종류의 파일(TS, JSX, 이미지, CSS 등)을 자동으로 처리하는<br /><strong>‘무설정(Zero-config)’ 번들러</strong>로 유명.  </p>
<h4 id="장점-3">장점</h4>
<ul>
...
<blockquote>
<p><strong>번들러(Bundler)는 여러 JS 모듈을 중심으로,
하나 또는 소수의 번들 파일로 묶고,
