<p>프론트엔드 개발을 하다 보면<br />렌더링 이후 애니메이션 효과나 크기 변화를 주어야 하는 경우가 자주 발생한다.</p>
<p>이때 어떤 CSS 속성을 사용하느냐에 따라<br />CPU에 큰 부담을 줄 수도 있고,<br />GPU를 활용해 훨씬 부드러운 애니메이션을 만들 수도 있다.</p>
<p>이 글에서는<br />브라우저 렌더링 과정과 함께<br />성능 최적화 관점에서 자주 헷갈리는 CSS 속성들을 정리해봤다.</p>
<hr />
<h2 id="브라우저-렌더링-과정-요약">브라우저 렌더링 과정 요약</h2>
<p>브라우저 렌더링 과정을 간단히 설명하면 다음과 같다.</p>
<ol>
...
<p>이 과정에서 자주 등장하는 용어가 <strong>Reflow(Layout)</strong>와 <strong>Paint</strong>다.</p>
<ul>
...
<p>이 과정이 반복되면 CPU 부담이 커지고,<br />애니메이션이 끊겨 보일 수 있다.</p>
<p>자세한 설명은<br /><a href="https://velog.io/@sunmins/%EC%9B%B9-%EA%B0%9C%EB%B0%9C-%EC%8A%A4%ED%82%AC%EC%9D%84-%ED%95%9C-%EB%8B%A8%EA%B3%84-%EB%86%92%EC%97%AC%EC%A3%BC%EB%8A%94-%ED%94%84%EB%A1%A0%ED%8A%B8%EC%97%94%EB%93%9C-%EC%84%B1%EB%8A%A5-%EC%B5%9C%EC%A0%81%ED%99%94-%EA%B0%80%EC%9D%B4%EB%93%9C-poxhygb3">해당 글</a>에서 확인할 수 있다.</p>
<hr />
<h2 id="gpu-가속을-사용하는-css-속성">GPU 가속을 사용하는 CSS 속성</h2>
<p>렌더링 이후 애니메이션이나 상태 변화가 발생하면<br />브라우저는 기본적으로 다시 <strong>Layout → Paint → Composite</strong> 과정을 거친다.</p>
<p>하지만 일부 CSS 속성은<br />Layout과 Paint 단계를 건너뛰고<br /><strong>Composite 단계만 다시 수행</strong>하게 만들 수 있다.</p>
<p>이때 화면의 이동·합성 작업을<br />CPU가 아닌 <strong>GPU에서 처리하게 되는데</strong>,  
이러한 방식을 <strong>GPU 가속</strong>이라고 부른다.</p>
<p>GPU 가속이란, 다시 계산하고 다시 그리는 작업을 CPU 대신 GPU에게 맡겨  렌더링 성능을 최적화하고 화면을 더 부드럽게 만드는 방법이다.</p>
<p>GPU는 이미 그려진 화면을 이동하거나 합성하는 작업을<br />CPU보다 훨씬 빠르게 처리할 수 있기 때문이다.(Composite 단계가 CPU보다 압도적으로 빠름)</p>
<p>대표적인 GPU 가속 CSS 속성으로는<br /><strong><code>transform</code>과 <code>opacity</code></strong>가 있다.</p>
<hr />
<h2 id="1-transform">1. transform</h2>
<p><strong>의미:</strong> 변환하다, 바꾸다<br />요소의 <strong>모양이나 위치를 시각적으로 변경</strong>하는 속성이다.</p>
...
Layout,Paint를 생략하고 Composite 단계만 재수행한다.</p>
<hr />
...
<p>transition은 애니메이션 그 자체가 아니라<br /><strong>상태가 바뀔 때 그 변화 과정을 부드럽게 만들어주는 규칙</strong>이다.</p>
<p>속성의 변화를 감시한다고 해서<br /><strong>속성 감지자</strong>라고 생각하면 이해하기 쉽다.</p>
<hr />
<h4 id="문법">문법</h4>
<pre><code class="language-css">transition: A 0.3s, B 0.3s;</code></pre>
<p>위 코드는 아래와 같은 의미다.</p>
...
<h3 id="❗-주의할점">❗ 주의할점</h3>
<p>opacity는 보이기만 안보이게 할뿐이다.
그래서 클릭이 가능하고, 포커스가 되며 DOM에 존재한다.</p>
<p>그래서 보통 이렇게 같이 사용한다.</p>
<pre><code class="language-css">opacity: 0;
...
<hr />
<h2 id="마무리">마무리</h2>
<p>이번 글에서는 <strong>GPU 가속</strong>의 개념과 그 필요성에 대해 정리해보았다.<br />GPU 가속은 화면을 다시 계산하고 그리는 작업을 CPU 대신 GPU에게 맡겨<br />렌더링 성능을 향상시키는 방식이다.</p>
<p>또한 헷갈리기 쉬운 <strong>transform, translate, transition</strong>의 차이점을 구분하며<br />각 속성이 어떤 역할을 하는지 살펴보았다.</p>
<p>마지막으로 <strong>transform과 opacity</strong>처럼 GPU 가속이 가능한 CSS 속성과<br /><code>width</code>, <code>display</code> 등 CPU 부담이 큰 속성을 비교해보았다.</p>
<p>이처럼 CSS 속성 선택은 단순한 스타일링을 넘어<br /><strong>렌더링 성능과 사용자 경험에 직접적인 영향을 준다.</strong></p>